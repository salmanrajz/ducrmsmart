...
use App\Models\mission_du_second;
use Illuminate\Console\Command;

class MissionDu2 extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'MissionDu2';
    ...
    public function handle()
    {
        ...
        for($i = 667; $i<2000; $i++){
            echo $i . '<br>';
            if($i == 0){
                echo $url = 'https://salmanrajzzdiag.blob.core.windows.net/callmax/mission_second/bgt-1.csv';
                mission_du_second::create([
                    'filename' => $url,
                ]);
            }
            // else if($i == 1){
            ...
            // }
                ...
                $url = 'https://salmanrajzzdiag.blob.core.windows.net/callmax/mission_second/bgt-' . $i . '.csv';
                mission_du_second::create([
                    'filename' => $url,
                ]);
            }
        ...
        if ($m) {
            ...
            if ($httpcode == 200) {
                ...
                $m = mission_du_second::where('filename', $link)->first();
                $m->status = 1;
                $m->save();
            } else {
                $details = [
